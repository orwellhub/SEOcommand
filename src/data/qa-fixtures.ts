...
export function qaLinkBuilding(siteSlug: string) {
  ...
  return { summary: { prospects: prospects.length, qualifiedProspects: prospects.filter((item) => item.quality.eligible).length, strongProspects: prospects.filter((item) => item.quality.state === "strong").length, awaitingApproval: drafts.filter((item) => item.status === "draft").length, sent: 0 }, prospects, drafts };
}
...
export function qaAiVisibility(scope: string, allowedSiteSlugs?: string[] | null) {
  ...
  return {
    ...
    summary: { checks: observations.length, mentions: observations.filter((r) => r.mentioned).length, citedResponses: observations.filter((r) => r.cited).length, citedPages: new Set(observations.flatMap((r) => r.citations.map((c) => c.url))).size, sitesMeasured: measuredSites.length, mentionRate: primary ? 86 : 0, citationRate: primary ? 71 : 0, avgRecommendationPosition: primary ? 2.1 : null, positiveSentimentRate: primary ? 83 : 0, shareOfVoice: primary ? 54 : 0 },
    trend: Array.from({ length: 14 }, (_, day) => ({ date: `2026-08-${String(day + 13).padStart(2, "0")}`, mentions: day + 2, citedResponses: day + 1, citedPages: 1, mentionRate: 62 + day * 1.8, citationRate: 48 + day * 1.6, shareOfVoice: 39 + day * 1.2 })),
    platforms: platforms.map((platform, index) => ({ platform, mentions: 1, citedResponses: 1, citedPages: 1, checks: Math.max(1, measuredSites.length), mentionRate: 92 - index * 4, citationRate: 78 - index * 5, avgPosition: 1.5 + index * 0.25 })),
    countries: [{ locationCode: "2784", checks: observations.length, mentions: observations.filter((r) => r.mentioned).length }],
    observations,
