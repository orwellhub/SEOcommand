...
export type ReportOutcome = { id: string; title: string; module: string; verification: { outcome?: string; confidence?: string; valueCreated?: { amount: number; currency: string; method: string } | null }; metrics: Array<{ label: string; percentChange: number | null }> };
export function ClientReport({ site, template, branding, bundle, outcomes = [] }: { site: Domain; template: ReportTemplate; branding: ReportBranding; bundle: DomainLiveBundle; outcomes?: ReportOutcome[] }) {
  ...
  return <article id="client-report" className="client-report mx-auto max-w-[940px] overflow-hidden bg-[#F7F8FB] text-[#11182B] shadow-[0_24px_80px_rgba(17,24,43,0.12)]" style={{ "--report-accent": branding.accent, "--report-secondary": branding.secondaryColor } as CSSProperties}>
    ...
    <div className="report-body space-y-10 px-6 py-10 sm:px-10 sm:py-12">
      ...
      {outcomes.length > 0 && <Section eyebrow="Measured delivery" title="Work completed and outcomes proven" summary="Completion and success are reported separately. Values marked estimated include the recorded assumption in SEOcommand’s outcome ledger."><div className="space-y-3"><div className="grid grid-cols-3 gap-px overflow-hidden rounded-md bg-[#DDE2EC]"><ReportStat label="Shipped actions" value={String(outcomes.length)} detail="With evidence trail" /><ReportStat label="Verified wins" value={String(outcomes.filter((item) => item.verification.outcome === "won").length)} detail="Measured outcomes" /><ReportStat label="Value created" value={outcomes.filter((item) => item.verification.valueCreated).map((item) => `${item.verification.valueCreated!.currency} ${compact(item.verification.valueCreated!.amount)}`).join(" + ") || "—"} detail="Recorded + estimated" /></div><ReportTable headers={["Delivered action", "Outcome", "Primary change", "Confidence"]} rows={outcomes.slice(0, 8).map((item) => [item.title, (item.verification.outcome ?? "Awaiting data").replace(/_/g, " "), item.metrics[0]?.percentChange == null ? "—" : `${item.metrics[0].percentChange! > 0 ? "+" : ""}${item.metrics[0].percentChange!.toFixed(1)}% ${item.metrics[0].label}`, item.verification.confidence ?? "—"])} /></div></Section>}
