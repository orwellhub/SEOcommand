...
export function ReportMetric({ label, value, note, href, compact=false }: { label: string; value?: string | number | null; note?: string; href?: string; compact?:boolean }) {
  const content = <><span className={compact?"block text-[11px] leading-4 text-muted":"block text-xs text-muted"}>{label}</span><span className={cn("mt-1 flex items-center gap-1 font-semibold tracking-tight tnum",compact?"text-xl leading-6":"text-2xl leading-8")}>{value ?? "—"}{href && <ArrowUpRight className="h-3.5 w-3.5 text-muted" />}</span><span className={cn("mt-0.5 block text-muted",compact?"text-[10px] leading-3":"text-[11px] leading-4")}>{value == null ? note ?? "Unavailable" : note ?? ""}</span></>;
  return href ? <Link href={href} className={cn("report-metric block min-w-0 transition-colors hover:bg-workspace",compact?"px-3 py-2":"px-4 py-3")}>{content}</Link> : <div className={cn("report-metric min-w-0",compact?"px-3 py-2":"px-4 py-3")}>{content}</div>;
}
