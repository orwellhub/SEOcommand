...
export function SiteScanCentre({ siteId }: { siteId: string }) {
  ...
  return <Card id="website-scan-centre" role="region" aria-label="Website Scan Centre" className="border-purple/25 border-l-4 border-l-purple bg-card px-4 py-2.5">
    <div className="flex flex-wrap items-center justify-between gap-3">
      ...
      <Link href={`/scan-centre?site=${encodeURIComponent(siteId)}`} className="inline-flex min-h-9 shrink-0 items-center gap-2 rounded-md border border-border px-3 py-1.5 text-sm font-semibold text-purple hover:bg-workspace">
