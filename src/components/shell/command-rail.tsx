...
export function CommandRail() {
  ...
  return <div className={cn("relative z-40 hidden shrink-0 transition-[width] duration-200 motion-reduce:transition-none lg:block", pinned ? "w-[304px]" : "w-[72px]")}>
    <aside aria-label="Toolkits" className="absolute inset-y-0 left-0 z-10 flex w-[72px] flex-col border-r border-border bg-card">
      <Link href="/portfolio?scope=portfolio" aria-label="SEO Command home" className="flex h-[66px] shrink-0 items-center justify-center"><BrandLogo variant="mark" className="w-11" /></Link>
    ...
    <div className={cn("absolute inset-y-0 left-[72px]", expanded ? "w-[232px]" : "w-2")} onPointerEnter={(event) => { if (event.pointerType !== "touch") setHovered(true); }} onPointerLeave={() => setHovered(false)} onFocusCapture={() => setFocused(true)} onBlurCapture={(event) => { if (!event.currentTarget.contains(event.relatedTarget)) setFocused(false); }}>
      {expanded && <aside aria-label={`${kit.label} tools`} className={cn("flex h-full flex-col border-r border-border bg-workspace", !pinned && "shadow-pop")}>
        <div className="flex h-[66px] shrink-0 items-center border-b border-border px-4"><span className="text-lg font-bold tracking-tight">{kit.id === "ai" ? "AI Visibility" : kit.label}</span></div>
        <div className="shrink-0 border-b border-border p-3"><ScopePicker label="Select website in sidebar" />{activeDomain && <Link href="/portfolio?scope=portfolio" className="mt-1.5 flex min-h-7 items-center gap-1.5 text-xs text-muted hover:text-purple"><ArrowLeft className="h-3 w-3" />All websites</Link>}</div>
        <nav id="command-navigation" aria-label={`${kit.label} features`} className="min-h-0 flex-1 overflow-y-auto px-2 pb-5 pt-2">{kit.groups.map((group, index) => <div key={group.label || index} className="mb-3">{group.label && <h2 className="px-2 pb-1 pt-2 text-[11px] font-bold tracking-wide text-muted">{group.label}</h2>}{group.items.map((entry) => <Link key={entry.href} href={navigationHref(entry, scope, range)} onNavigate={finish} aria-current={selected?.href === entry.href ? "page" : undefined} className={cn("flex min-h-8 items-center rounded px-2 py-1.5 text-[13px] leading-5", selected?.href === entry.href ? "bg-rail-selected font-bold text-purple" : "text-ink hover:bg-card hover:text-purple")}>{entry.label}</Link>)}</div>)}</nav>
      </aside>}
    </div>
  </div>;
}
