...
export function CommandRail() {
  ...
  return <div className={cn("relative z-40 hidden shrink-0 transition-[width] duration-200 motion-reduce:transition-none lg:block", pinned ? "w-[232px]" : "w-[56px]")}>
    <aside aria-label="Toolkits" className="absolute inset-y-0 left-0 z-10 flex w-[56px] flex-col border-r border-border bg-card">
      <Link href="/portfolio?scope=portfolio" aria-label="SEO Command home" className="flex h-11 shrink-0 items-center justify-center"><BrandLogo variant="mark" className="w-9" /></Link>
      <nav className="min-h-0 flex-1 overflow-y-auto px-1.5 py-2">{TOOLKITS.map((entry) => <Link key={entry.id} href={navigationHref({ href: entry.href, group: "site" }, scope, range)} aria-current={kit.id === entry.id ? "true" : undefined} className={cn("mb-1 flex min-h-[54px] flex-col items-center justify-center gap-1 rounded-md px-0.5 py-1.5 text-center text-[10px] font-semibold leading-[14px] transition-colors", kit.id === entry.id ? "bg-rail-selected text-purple" : "text-muted hover:bg-workspace hover:text-ink")}><entry.icon className="h-5 w-5" strokeWidth={1.7} /><span>{entry.label}</span></Link>)}</nav>
    ...
    <div className={cn("absolute inset-y-0 left-[56px]", expanded ? "w-[176px]" : "w-2")} onPointerEnter={(event) => { if (event.pointerType !== "touch") setHovered(true); }} onPointerLeave={() => setHovered(false)} onFocusCapture={() => setFocused(true)} onBlurCapture={(event) => { if (!event.currentTarget.contains(event.relatedTarget)) setFocused(false); }}>
      {expanded && <aside aria-label={`${kit.label} tools`} className={cn("flex h-full flex-col border-r border-border bg-workspace", !pinned && "shadow-pop")}>
        <div className="flex h-11 shrink-0 items-center px-3"><span className="text-sm font-bold">{kit.id === "ai" ? "AI Visibility" : kit.label}</span></div>
        <nav id="command-navigation" aria-label={`${kit.label} features`} className="min-h-0 flex-1 overflow-y-auto px-2 pb-5 pt-2">{kit.groups.map((group, index) => <div key={group.label || index} className="mb-2">{group.label && <h2 className="px-2 pb-1 pt-2 text-[11px] font-medium text-muted">{group.label}</h2>}{group.items.map((entry) => <Link key={entry.href} href={navigationHref(entry, scope, range)} onNavigate={finish} aria-current={selected?.href === entry.href ? "page" : undefined} className={cn("flex min-h-7 items-center rounded px-2 py-1 text-[13px] leading-[18px]", selected?.href === entry.href ? "bg-rail-selected font-bold text-purple" : "text-ink hover:bg-card hover:text-purple")}>{entry.label}</Link>)}</div>)}</nav>
      </aside>}
    </div>
  </div>;
}
