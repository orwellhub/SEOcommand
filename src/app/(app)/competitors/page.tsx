  ...
  return <div className="animate-in space-y-5">
    ...
    <Card className="overflow-hidden">
      <div className="grid lg:grid-cols-[1fr_auto]">
        <div className="p-5"><div className="text-2xs font-medium uppercase tracking-wide text-muted">Competitor domain</div><div className="mt-2 flex gap-2"><div className="relative flex-1"><Search className="absolute left-3 top-2.5 h-4 w-4 text-muted" /><input value={target} onChange={(event) => setTarget(event.target.value)} onKeyDown={(event) => event.key === "Enter" && void explore()} placeholder="competitor.com" className="h-9 w-full rounded-md border border-border bg-card pl-9 pr-3 text-sm outline-none focus:border-purple" /></div><Button variant="primary" onClick={explore} disabled={busy || !target.trim()}>{busy ? "Scanning…" : "Explore"}<ArrowRight className="h-4 w-4" /></Button></div>{error && <p role="alert" className="mt-2 text-xs text-critical">{error}</p>}<p className="mt-2 text-2xs text-muted">Runs four cost-guarded DataForSEO datasets and stores the evidence for reuse.</p></div>
