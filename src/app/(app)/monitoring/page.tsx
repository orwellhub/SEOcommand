...
import { Activity, Clock3, FileText, LockKeyhole, Radio, RefreshCw } from "lucide-react";
...
/**
 * llms.txt is recorded, not graded: Google states no AI-specific file is needed,
 * so an absent file is neutral rather than a fault.
 */
function llmsState(row: CheckRow): { label: string; tone: "success" | "warning" | "neutral" } {
  const llms = row.details?.llms;
  if (!llms) return { label: "Not checked", tone: "neutral" };
  if (!llms.present) return { label: "Absent", tone: "neutral" };
  return llms.valid ? { label: "Present", tone: "success" } : { label: "Malformed", tone: "warning" };
}
...
export default function MonitoringPage() {
  ...
  const columns = useMemo<Column<CheckRow>[]>(() => [
    ...
    { key: "llms", header: "llms.txt", sortValue: (row) => llmsState(row).label, render: (row) => { const state = llmsState(row); return <StatusBadge label={state.label} tone={state.tone} />; } },
