...
function CrawlerAccess({ data }: { data: AiVisibilityDashboard }) {
  return <Card className="p-4"><div className="mb-3"><h2 className="text-sm font-semibold text-ink">AI crawler access</h2><p className="mt-0.5 text-2xs text-muted">Latest robots.txt policy observed for search, assistant and training agents. Open a website for its full AI readiness report. Each token is matched against the root and the pages with the most search traffic; “partial” means the root is reachable but some of those pages are not.</p></div>{data.crawlerAudit.length ? <div className="grid gap-2 md:grid-cols-2 xl:grid-cols-3">{data.crawlerAudit.map((row) => <div key={`${row.siteSlug}-${row.bot}`} className="rounded-md border border-border p-3"><div className="flex items-start justify-between gap-2"><div><div className="font-medium text-ink">{row.bot}</div><div className="text-2xs text-muted"><Link href={`/health?site=${encodeURIComponent(row.siteSlug)}&view=ai`} className="text-purple hover:underline">{row.siteName}</Link> · {row.category}</div></div><StatusBadge label={row.access} tone={row.access === "allowed" ? "success" : row.access === "blocked" ? (row.category === "training" ? "warning" : "critical") : row.access === "partial" ? "warning" : "neutral"} /></div><p className="mt-3 text-xs leading-relaxed text-muted">{row.evidence}</p>{row.checkedPages != null && <p className="mt-1 text-2xs text-muted tnum">{row.blockedPages ?? 0} of {row.checkedPages} checked paths disallowed</p>}</div>)}</div> : <EmptyState title="Crawler audit not recorded yet" description="The daily worker checks each website’s robots.txt without consuming provider credits." icon={<Bot className="h-5 w-5" />} />}</Card>;
}
