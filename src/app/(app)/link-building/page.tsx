...
interface Prospect { id: string; sourceDomain: string; sourceUrl: string | null; authority: number | null; relevance: number; reason: string; competitorHosts: string[]; contacts: Array<{ type?: string; value?: string }>; status: string; traffic: number | null; quality: { state: "strong" | "qualified" | "review"; eligible: boolean; reasons: string[] }; contactState: string; outreachStatus: string }
...
interface LinkData { summary: { prospects: number; qualifiedProspects?: number; strongProspects: number; awaitingApproval: number; sent: number }; prospects: Prospect[]; drafts: Draft[] }
...
export default function LinkBuildingPage() {
  ...
  const gapMode = params.get("view") === "gap";
  ...
  const prospectColumns = useMemo<Column<Prospect>[]>(() => [
    ...
    { key: "quality", header: "Quality", render: (row) => <StatusBadge label={row.quality?.state ?? "review"} tone={row.quality?.state === "strong" ? "success" : row.quality?.eligible ? "info" : "warning"} /> },
    { key: "authority", header: "Authority", align: "right", sortValue: (row) => row.authority ?? 0, render: (row) => row.authority ?? "—" },
    { key: "traffic", header: "Traffic", align: "right", render: () => <span className="text-muted">Not measured</span> },
    ...
    { key: "action", header: "", align: "right", render: (row) => <Button size="sm" onClick={() => { setSelected(row); setRecipient(row.contacts.find((item) => item.type === "email")?.value ?? ""); }}>{row.quality?.eligible ? "Prepare outreach" : "Review quality"}</Button> },
  ], []);
  ...
  return <div className="animate-in space-y-5">
    <PageHeader title={`${gapMode ? "Backlink Gap" : "Link Building"}: ${domain.host}`} description={gapMode ? "Compare competitor referring domains and identify saved link opportunities for your website." : "Discover prospects, review outreach and verify acquired links."} />
    ...
    <Card id="link-gap"><div className="grid lg:grid-cols-[1fr_auto]"><div className="p-4"><div className="text-xs font-semibold text-ink">Find link gaps</div><p className="mt-1 text-2xs text-muted">Enter competitors that already rank in this market. Existing links to {domain.host} are excluded.</p><div className="mt-3 flex gap-2"><div className="relative flex-1"><Search className="absolute left-3 top-2.5 h-4 w-4 text-muted" /><input aria-label="Competitor domains for link gap" className={`${inputClass} pl-9`} value={competitors} onChange={(event) => setCompetitors(event.target.value)} placeholder="competitor-one.com, competitor-two.com" /></div><Button variant="primary" onClick={discover} disabled={busy === "discover" || !competitors.trim()}>{busy === "discover" ? "Finding…" : "Find prospects"}</Button></div></div><div className="flex items-center gap-3 border-t border-border bg-workspace/50 px-5 py-4 lg:w-80 lg:border-l lg:border-t-0"><ShieldCheck className="h-5 w-5 shrink-0 text-success" /><p className="text-2xs leading-relaxed text-muted">No outreach is sent until a user reviews and approves the exact recipient and message.</p></div></div></Card>
    {gapMode ? <Card className="p-4"><CardHeader title="Referring domain opportunities" subtitle="Saved competitor links; known existing links were excluded during discovery. Counts describe the collected sample."/><div className="mb-4 flex flex-wrap gap-2">{[["all","All opportunities"],["shared","Links to multiple competitors"],["unique","One competitor"],["qualified","Qualified"]].map(([id,label])=><Button key={id} size="sm" variant={gapFilter===id?"primary":"secondary"} onClick={()=>setGapFilter(id)}>{label}</Button>)}<Link className="ml-auto text-sm text-purple" href={`/link-building?site=${domain.id}&range=${params.get("range")??"28d"}`}>Open outreach →</Link></div><DataTable rows={(data?.prospects??[]).filter(row=>gapFilter==="all"||gapFilter==="shared"&&row.competitorHosts.length>1||gapFilter==="unique"&&row.competitorHosts.length===1||gapFilter==="qualified"&&row.quality.eligible)} rowKey={row=>row.id} searchKeys={row=>`${row.sourceDomain} ${row.competitorHosts.join(" ")}`} exportName="backlink-gap" columns={[{key:"domain",header:"Referring domain",sortValue:row=>row.sourceDomain,render:row=>row.sourceDomain},{key:"rank",header:"Provider rank",sortValue:row=>row.authority??-1,render:row=>row.authority??"—"},...[...new Set(data?.prospects.flatMap(row=>row.competitorHosts)??[])].slice(0,4).map(host=>({key:host,header:host,render:(row:Prospect)=>row.competitorHosts.includes(host)?"Link observed":"Not in sample"})),{key:"evidence",header:"Competitor domains",sortValue:row=>row.competitorHosts.length,render:row=>row.competitorHosts.join(", ")},{key:"action",header:"Action",render:row=><Button size="sm" onClick={()=>setSelected(row)}>Review prospect</Button>}]} emptyLabel="Enter competitors above to collect a backlink gap report, or reopen saved opportunities for this website."/></Card> : <><Card><CardHeader title="Prospects & messages" subtitle="Evidence and messages stay connected to the originating prospect" action={<Link2 className="h-4 w-4 text-purple" />} /><div className="flex gap-1 border-b border-border px-4 py-2"><button onClick={() => setTab("prospects")} className={`rounded-md px-3 py-1.5 text-xs font-medium ${tab === "prospects" ? "bg-purple text-white" : "text-muted hover:bg-workspace"}`}>Prospects ({data?.prospects.length ?? 0})</button><button onClick={() => setTab("drafts")} className={`rounded-md px-3 py-1.5 text-xs font-medium ${tab === "drafts" ? "bg-purple text-white" : "text-muted hover:bg-workspace"}`}>Messages ({data?.drafts.length ?? 0})</button></div>{tab === "prospects" ? <DataTable<Prospect> rows={data?.prospects??[]} columns={prospectColumns} searchKeys={row=>`${row.sourceDomain} ${row.reason}`} searchPlaceholder="Search prospects…" rowKey={(row) => row.id} emptyLabel="Discover prospects using competitors above."/> : <DataTable<Draft> rows={data?.drafts??[]} columns={draftColumns} searchKeys={row=>`${row.subject} ${row.recipientEmail}`} searchPlaceholder="Search messages…" rowKey={(row) => row.id} emptyLabel="Choose a prospect to prepare a draft."/>}</Card><OutreachDesk /></>}
    <Drawer open={Boolean(selected)} onClose={() => setSelected(null)} title={selected ? `Prepare outreach to ${selected.sourceDomain}` : "Prepare outreach"} subtitle="Qualification, evidence and approval stay connected" footer={<div className="flex justify-end gap-2"><Button onClick={() => { if (!selected) return; setWorkFinding({ key: `link-prospect:${selected.id}`, title: `Pursue link opportunity with ${selected.sourceDomain}`, module: "Backlinks", executionType: "link_prospect_list", priorityScore: selected.relevance, pageMode: "site_wide", evidenceLabel: `Authority ${selected.authority ?? "unavailable"} · fit ${selected.relevance} · ${selected.competitorHosts.length} competitor links`, sourceUrl: `/link-building?site=${encodeURIComponent(domain.id)}`, sourceEvidence: { prospectId: selected.id, sourceDomain: selected.sourceDomain, authority: selected.authority, relevance: selected.relevance, competitorHosts: selected.competitorHosts, quality: selected.quality }, }); setSelected(null); }}>Create execution work</Button><Button variant="primary" onClick={createDraft} disabled={!selected?.quality?.eligible || busy === `draft:${selected?.id}`}>{busy === `draft:${selected?.id}` ? "Creating…" : "Create draft"}</Button></div>}>
      {selected && <div className="space-y-5"><div className="rounded-md border border-border bg-workspace/50 p-3"><div className="flex items-center justify-between"><div className="text-xs font-semibold text-ink">Quality decision</div><StatusBadge label={selected.quality?.state ?? "review"} tone={selected.quality?.eligible ? "success" : "warning"} /></div><p className="mt-1 text-xs leading-relaxed text-muted">Minimum: fit 60, authority 20 and one competitor link. Strong: fit 70, authority 40 and two competitor links.</p>{selected.quality?.reasons?.length ? <p className="mt-2 text-2xs font-semibold text-warning">{selected.quality.reasons.join(" · ")}</p> : null}</div><div className="rounded-md border border-border bg-workspace/50 p-3"><div className="text-xs font-semibold text-ink">Why it is relevant</div><p className="mt-1 text-xs leading-relaxed text-muted">{selected.reason}</p><p className="mt-2 text-2xs text-muted">Evidence: {selected.competitorHosts.join(", ") || "none"}</p></div><div><div className="flex items-center justify-between"><label className="text-xs font-medium text-ink">Public contact evidence</label><Button size="sm" onClick={enrich} disabled={busy === `contacts:${selected.id}`}><Search className="h-3.5 w-3.5" />{busy === `contacts:${selected.id}` ? "Researching…" : "Research contacts"}</Button></div><div className="mt-2 space-y-1">{selected.contacts.length ? selected.contacts.map((contact, index) => <div key={`${contact.type}:${contact.value}:${index}`} className="rounded bg-workspace px-2.5 py-2 text-xs text-muted">{contact.type?.replace(/_/g, " ")}: {contact.value}</div>) : <p className="text-2xs text-muted">No contact research has been run.</p>}</div></div><label className="block text-xs font-medium text-ink">Recipient email<input className={`mt-1.5 ${inputClass}`} value={recipient} onChange={(event) => setRecipient(event.target.value)} placeholder="editor@example.com" /></label><label className="block text-xs font-medium text-ink">Why your resource belongs<textarea className="mt-1.5 min-h-28 w-full rounded-md border border-border bg-card px-3 py-2 text-sm outline-none focus:border-purple" value={angle} onChange={(event) => setAngle(event.target.value)} placeholder="A specific, truthful reason this resource improves their page" /></label><div className="rounded-md border border-warning/25 bg-warning/5 p-3 text-2xs leading-relaxed text-muted">Creating a draft does not send it. The message must be approved separately before the delivery action becomes available.</div></div>}
    </Drawer>
