...
export default function LinkBuildingPage() {
  ...
  const enrich = async () => {
    if (!selected) return;
    setBusy(`contacts:${selected.id}`); setError(null);
    try {
  ...
  const createDraft = async () => {
    if (!selected) return;
    setBusy(`draft:${selected.id}`); setError(null);
    try {
      ...
      setSelected(null); setRecipient(""); setAngle(""); setTab("drafts"); await load();
  ...
  const draftAction = useCallback(async (draft: Draft, action: "approve" | "send") => {
    setBusy(`${action}:${draft.id}`); setError(null);
    try {
