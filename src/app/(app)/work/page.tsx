...
import { scopedSiteIds } from "@/lib/site-context";
...
export default function WorkPage() {
  const { sites, scope: websiteScope, groups } = useDomain();
  const router = useRouter();
  const dismissedItem = useRef<string | null>(null);
  const searchParams = useSearchParams(),projectId=searchParams.get("project");
  ...
  useEffect(() => {
    ...
    const item = requested && dismissedItem.current !== requested ? items.find((candidate) => candidate.id === requested && scopedSiteIds(websiteScope, sites, groups).has(candidate.domainSlug)) : null;
    if (item && selected?.id !== item.id) void openItem(item);
  }, [items, searchParams, selected?.id, websiteScope, sites, groups]);
  useEffect(() => { setSelected(null); }, [websiteScope]);

  const scopedItems = useMemo(() => items.filter((item) => scopedSiteIds(websiteScope, sites, groups).has(item.domainSlug)&&(!projectId||item.sourceEvidence.projectId===projectId)), [items, websiteScope, sites, groups,projectId]);
  const visible = useMemo(() => scope === "mine" ? scopedItems.filter((item) => item.ownerEmail?.toLowerCase() === userEmail) : scopedItems, [scopedItems, scope, userEmail]);
  function closeItem() {
    dismissedItem.current = searchParams.get("item");
    setSelected(null);
    const params = new URLSearchParams(searchParams); params.delete("item");
    router.replace(`/work${params.size ? `?${params}` : ""}`, { scroll: false });
  }
  ...
  async function openItem(item: WorkflowItem) {
    ...
    try {
      ...
      if (!response.ok) throw new Error("Task history could not load. Close and reopen to retry.");
      setComments(body.comments ?? []); setHistory(body.history ?? []);
    } catch (reason) { setError(reason instanceof Error ? reason.message : "Task history could not load."); }
  }
  ...
  return <div className="animate-in space-y-6">
    ...
    {projectId&&<div className="flex flex-wrap gap-3 text-xs"><span>Filtered to the selected research project</span><Link className="text-purple" href={`/keyword-research?site=${searchParams.get("site")??""}&view=projects&project=${projectId}`}>Back to research project →</Link><button className="text-purple" onClick={()=>{const query=new URLSearchParams(searchParams);query.delete("project");router.replace(`/work?${query}`);}}>Show all website work</button></div>}
    ...
    {loading ? <div className="grid gap-4 lg:grid-cols-5">{STAGES.map((stage) => <Skeleton key={stage} className="h-64" />)}</div> : visible.length === 0 ? <Card className="p-6"><EmptyState title={scope === "mine" ? "No work assigned to you" : "No approved work yet"} description="Approve a mapped opportunity in Action Centre, or change the ownership filter." icon={<CheckCircle2 className="h-7 w-7 text-success" />} /></Card> : <div className="grid gap-4 xl:grid-cols-5">{STAGES.map((stage) => { const rows = visible.filter((item) => (item.status ?? "approved") === stage); return <section key={stage} className="min-w-0 rounded-lg border border-border bg-workspace/45"><div className="flex items-center justify-between border-b border-border px-3 py-3"><div className="text-xs font-bold text-ink">{STAGE_LABELS[stage]}</div><span className="flex h-5 min-w-5 items-center justify-center rounded-full bg-card px-1 text-[12px] font-bold text-muted">{rows.length}</span></div><div className="space-y-2 p-2"><Paginated label={`${STAGE_LABELS[stage]} work`} pageSize={8}>{rows.map((item) => <button key={item.id} onClick={() => void openItem(item)} className="w-full rounded-md border border-border bg-card p-3 text-left shadow-sm transition hover:-translate-y-0.5 hover:border-purple/35 hover:shadow-card"><div className="flex items-center justify-between gap-2"><StatusBadge label={formatType(item.executionType)} tone="info" /><span className="text-[12px] font-bold text-purple tnum">{item.priorityScore}</span></div><div className="mt-2 text-xs font-bold leading-5 text-ink">{item.title}</div><div className="mt-3 space-y-1 text-[12px] text-muted"><div className="flex items-center gap-1.5"><UserRound className="h-3 w-3" />{item.ownerEmail ?? "Unassigned"}</div><div className="flex items-center gap-1.5"><CalendarDays className="h-3 w-3" />{displayDate(item.dueDate)}</div><div className="truncate">{siteName(item.domainSlug)}</div></div></button>)}</Paginated>{rows.length === 0 && <div className="px-2 py-8 text-center text-[12px] text-muted">No items</div>}</div></section>; })}</div>}

    {selected && <Drawer open onClose={closeItem} title={selected.title} subtitle={`${siteName(selected.domainSlug)} · ${formatType(selected.executionType)}`} width="max-w-2xl" footer={<div className="flex items-center justify-between gap-3"><StatusBadge label={STAGE_LABELS[currentStage]} tone={STAGE_TONES[currentStage]} />{currentStage === "verifying" && !outcomeReviewed ? <span className="text-xs font-semibold text-muted">Review a checkpoint and classify the outcome above</span> : nextStage ? <Button variant="primary" onClick={() => void updateItem({ status: nextStage }, "stage")} disabled={busy === "stage"}>{busy === "stage" && <Loader2 className="h-4 w-4 animate-spin" />}Move to {STAGE_LABELS[nextStage]}</Button> : <span className="inline-flex items-center gap-1 text-xs font-bold text-success"><CheckCircle2 className="h-4 w-4" />Verification complete</span>}</div>}>
      <div className="space-y-6">
        <div className="grid grid-cols-5 gap-1">{STAGES.map((stage, index) => <div key={stage}><div className={cn("h-1.5 rounded-full", index <= STAGES.indexOf(currentStage) ? "bg-purple" : "bg-workspace")} /><div className="mt-1 truncate text-[12px] font-semibold text-muted">{STAGE_LABELS[stage]}</div></div>)}</div>

        <section className="rounded-lg border border-border bg-workspace/45 p-4"><div className="text-2xs font-bold uppercase tracking-[0.12em] text-muted">Evidence and destination</div><div className="mt-3 grid gap-3 sm:grid-cols-2"><div><div className="text-[12px] font-semibold text-muted">Research source</div><div className="mt-1 text-sm font-bold text-ink">{String(selected.sourceEvidence?.sourceValue ?? "Attached evidence")}</div><div className="mt-0.5 text-xs text-muted">{(typeof selected.sourceEvidence?.market==="string"?selected.sourceEvidence.market:typeof selected.sourceEvidence?.market==="object"&&selected.sourceEvidence.market?String((selected.sourceEvidence.market as {label?:string}).label??""):"")}</div></div><div><div className="text-[12px] font-semibold text-muted">Destination</div><div className="mt-1 break-all text-sm font-bold text-ink">{selected.targetUrl ?? selected.plannedUrl ?? "Site-wide"}</div><div className="mt-0.5 text-xs capitalize text-muted">{formatType(selected.pageMode)}</div></div></div>{selected.sourceUrl && <Link href={selected.sourceUrl} className="mt-3 inline-flex items-center gap-1 text-xs font-bold text-purple">Open original evidence <ExternalLink className="h-3.5 w-3.5" /></Link>}</section>
