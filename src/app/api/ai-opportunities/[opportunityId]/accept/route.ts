...
export async function POST(request: Request, { params }: { params: Promise<{ opportunityId: string }> }) {
  ...
  if (process.env.QA_SYNTHETIC === "true") {
    if (!await hasPermission(request, "manage_content")) return NextResponse.json({ error: "AI prompt permission required." }, { status: 403 });
    return NextResponse.json({ prompt: { id: "51000000-0000-4000-8000-000000000098", ...parsed.data, active: true, synthetic: true }, accepted: true, opportunityId });
  }
  ...
  if (!await hasPermission(request, "manage_content", site.id)) return NextResponse.json({ error: "AI prompt permission required for this website." }, { status: 403 });
