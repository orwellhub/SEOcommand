...
export async function GET(request: Request, { params }: { params: { siteId: string } }) {
  ...
  const [run] = await db().select().from(schema.detailedCrawlRuns).where(eq(schema.detailedCrawlRuns.siteSlug, params.siteId)).orderBy(desc(schema.detailedCrawlRuns.startedAt)).limit(1);
