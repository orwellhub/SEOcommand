...
function response(rows: ReturnType<typeof buildOutcomeRow>[]) {
  ...
  const now = new Date();
  const awaiting = rows.filter((row) => !row.proof.verified);
  const due = awaiting.filter((row) => row.verification.checkpoints?.some((checkpoint) => checkpoint.status === "scheduled" && new Date(checkpoint.dueAt) <= now));
  const needsReview = awaiting.filter((row) => row.verification.checkpoints?.some((checkpoint) => checkpoint.status === "recorded"));
  const nextCheckAt = awaiting.flatMap((row) => row.verification.checkpoints ?? []).filter((checkpoint) => checkpoint.status === "scheduled" && new Date(checkpoint.dueAt) > now).map((checkpoint) => checkpoint.dueAt).sort()[0] ?? null;
  return { rows, learning, queue: { due: due.length, needsReview: needsReview.length, nextCheckAt, comparableGroups: learning.filter((signal) => signal.samples >= 3).length }, summary: { total: rows.length, shipped: rows.filter((row) => row.proof.shipped).length, verified: rows.filter((row) => row.proof.verified).length, won: rows.filter((row) => row.verification.outcome === "won").length, lost: rows.filter((row) => row.verification.outcome === "lost").length, inconclusive: rows.filter((row) => row.verification.outcome === "inconclusive").length, awaitingData: awaiting.length, valueByCurrency: Object.entries(values.reduce<Record<string, number>>((all, value) => { if (value) all[value.currency] = (all[value.currency] ?? 0) + value.amount; return all; }, {})).map(([currency, amount]) => ({ currency, amount })) } };
}
