...
export function metricDeltas(verification: VerificationState): MetricDelta[] {
  ...
  return (verification.baseline?.metrics ?? []).map((baseline) => {
    ...
    const absoluteChange = current == null ? null : current - baseline.value;
...
export function buildLearningSignals(items: OutcomeWork[]): LearningSignal[] {
  ...
  return [...groups.values()].map((group) => { const wins = group.outcomes.filter((item) => item === "won").length; const losses = group.outcomes.filter((item) => item === "lost").length; const adjustment = group.outcomes.length < 2 ? 0 : Math.max(-10, Math.min(10, Math.round((wins - losses) / group.outcomes.length * 10))); return { domainSlug: group.domainSlug, executionType: group.executionType, samples: group.outcomes.length, wins, losses, adjustment, explanation: group.outcomes.length < 2 ? "No adjustment until two verified outcomes exist." : `${wins} won and ${losses} lost across ${group.outcomes.length} verified actions.` }; });
}
